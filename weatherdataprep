...
KNMI_10MIN_CSV = Path("KNMI_10min.csv") 
KNMI_WBGT_CSV  = Path("KNMI_WBGT.csv") 
...
OUT_10MIN = Path("knmi_meteo_filtered.csv")
OUT_WBGT  = Path("knmi_wbgt_filtered.csv")
OUT_3MIN  = Path("knmi_meteo_wbgt_3min.csv")
...
if not met.empty:
    ...
    # T (0.1 °C) -> °C, RH (0.1 %) -> %, FF (0.1 m/s) -> m/s, P (0.1 hPa) -> hPa, DR (0.1 mm) -> mm
    def safe_div(x, d):
        return pd.to_numeric(x, errors="coerce")/d

    T  = safe_div(met.get("T"), 10)    # °C
...
if not wb.empty:
    ...
    WBGT = pd.to_numeric(wb.get("WBGT"), errors="coerce")  # °C
    wb_out = pd.DataFrame({
